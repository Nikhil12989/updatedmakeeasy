...
const Application_statefoodlicense = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchLicenseData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/stateFoodLicense/getStateFoodLicense/${id}`);
  ...
  const handleUpdate = async (event) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:5000/api/stateFoodLicense/updatestatefoodlicense/${id}`,
        formData,
        {
          headers: {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto bg-gray-50 rounded-lg shadow-lg">
      <h1 className="text-4xl font-extrabold text-center text-gray-800 mb-8">State Food License Application</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
        <div className="bg-white shadow-md p-6 rounded-lg space-y-4">
          ...
          <p className="text-gray-800"><strong className="text-gray-600">Business Name:</strong> {licenseData.businessName}</p>
          <p className="text-gray-800"><strong className="text-gray-600">Nature of Business:</strong> {licenseData.natureOfBusiness}</p>
          <p className="text-gray-800"><strong className="text-gray-600">Qualification:</strong> {licenseData.ownerQualification}</p>
          ...
          <p className="text-gray-800"><strong className="text-gray-600">Business Address:</strong> {licenseData.businessAddress}</p>
          <p className="text-gray-800"><strong className="text-gray-600">Status:</strong> {licenseData.Status}</p>
        </div>

        <div className="bg-white shadow-md p-6 rounded-lg space-y-4">
          <h2 className="text-lg font-semibold mb-4 text-gray-700 border-b pb-2">Important Dates</h2>
          <p className="text-gray-800"><strong className="text-gray-600">Submitted At:</strong> {licenseData.submitedAt ? new Date(licenseData.submitedAt).toLocaleDateString() : 'N/A'}</p>
          <p className="text-gray-800"><strong className="text-gray-600">Rejected At:</strong> {licenseData.rejectedAt ? new Date(licenseData.rejectedAt).toLocaleDateString() : 'N/A'}</p>
          <p className="text-gray-800"><strong className="text-gray-600">Completed At:</strong> {licenseData.CompletedAt ? new Date(licenseData.CompletedAt).toLocaleDateString() : 'N/A'}</p>
        </div>
      </div>
...
export default Application_statefoodlicense;

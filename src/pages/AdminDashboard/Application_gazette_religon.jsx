...
const Application_gazette_religon = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchLicenseData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/gazetteReligionChange/getbyidgazette-religion-change/${id}`);
  ...
  const handleUpdate = async (event) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:5000/api/gazetteReligionChange//updategazette-religion-change/${id}`,
        formData,
        {
          headers: {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto bg-gray-50 rounded-lg shadow-lg">
      <h1 className="text-4xl font-extrabold text-center text-gray-800 mb-8">Gazette Religion Change Application</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
        <div className="bg-white shadow-md p-6 rounded-lg space-y-4">
          ...
          <p className="text-gray-800"><strong className="text-gray-600">Old Religion:</strong> {licenseData.oldReligion}</p>
          <p className="text-gray-800"><strong className="text-gray-600">New Religion:</strong> {licenseData.newReligion}</p>
          <p className="text-gray-800"><strong className="text-gray-600">Reason for Change:</strong> {licenseData.reasonForChange}</p>
        </div>

        <div className="bg-white shadow-md p-6 rounded-lg space-y-4">
          <h2 className="text-lg font-semibold mb-4 text-gray-700 border-b pb-2">Status Information</h2>
...
export default Application_gazette_religon;

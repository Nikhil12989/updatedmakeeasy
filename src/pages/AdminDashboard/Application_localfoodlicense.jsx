...
const Application_localfoodlicense = () => {
  ...
  const handleUpdate = async (event) => {
    ...
    for (const [key, value] of Object.entries(files)) {
      formData.append(`documents.${key}`, value); // fixed string interpolation here
    }
  ...
  const renderImage = (imageData) => {
    if (imageData && imageData.contentType && imageData.data && imageData.data.data) {
      ...
      return `data:${imageData.contentType};base64,${base64String}`; // fixed string interpolation here
    }
    return null;
  };
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto bg-gray-50 rounded-lg shadow-lg">
      <h1 className="text-4xl font-extrabold text-center text-gray-800 mb-8">Local food license Application</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
        <div className="bg-white shadow-md p-6 rounded-lg space-y-4">
          ...
          <p className="text-gray-800"><strong className="text-gray-600">Father/Husband Name:</strong> {licenseData.fatherFullName}</p>
        </div>

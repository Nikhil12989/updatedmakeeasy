...
const Application_votternew = () => {
    ...
    useEffect(() => {
      const fetchLicenseData = async () => {
        try {
          const response = await axios.get(`http://localhost:5000/api/newVoterID/getbyidnew-voter-id/${id}`);
    ...
    const handleUpdate = async (event) => {
      ...
      try {
        const response = await axios.put(
          `http://localhost:5000/api/newVoterID/updatenew-voter-id/${id}`,
          formData,
          {
            headers: {
      ...
      <div className="p-8 max-w-5xl mx-auto bg-gray-50 rounded-lg shadow-lg">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
          <div className="bg-white shadow-md p-6 rounded-lg space-y-4">
            ...
            <p className="text-gray-800"><strong className="text-gray-600">Birth State:</strong> {licenseData.birthState}</p>
            <p className="text-gray-800"><strong className="text-gray-600">Birth District:</strong> {licenseData.birthDistrict}</p>
            <p className="text-gray-800"><strong className="text-gray-600">Relation:</strong> {licenseData.relation}</p>
            <p className="text-gray-800"><strong className="text-gray-600">Relation Name:</strong> {licenseData.relationName}</p>
          </div>
          ...
          <div className="bg-white shadow-md p-6 rounded-lg space-y-4">
            ...
            <p className="text-gray-800"><strong className="text-gray-600">Relation Address:</strong> {licenseData.relationAddress}</p>
            <p className="text-gray-800"><strong className="text-gray-600">Relation Pincode:</strong> {licenseData.relationPincode}</p>
            <p className="text-gray-800"><strong className="text-gray-600">Living Address Since:</strong> {licenseData.livingAddressSince}</p>
          </div>
        </div>
...
export default Application_votternew;
